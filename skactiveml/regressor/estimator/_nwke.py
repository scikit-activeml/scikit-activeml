...
class NormalInverseWishartKernelEstimator(SkactivemlConditionalEstimator):
    ...
    def estimate_post(self, X):
        ...
        scatters = diff.reshape(n_cand, n_samples, d_t, 1) * diff.reshape(
            n_cand, n_samples, 1, d_t
        )
        S_L = S_0.reshape(1, d_t, d_t) + np.sum(
            K.reshape(n_cand, n_samples, 1, 1) * scatters, axis=1
        )
    ...
    def estimate_mu_cov(self, X):
        ...
        return (
            mu_L,
            ((kappa_L + 1) / ((nu_L - d_tar - 1) * kappa_L)).reshape(n_cand, 1, 1)
            * S_L,
        )
