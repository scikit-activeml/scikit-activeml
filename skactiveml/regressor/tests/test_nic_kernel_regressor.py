...
class TestNICKernelEstimator(TemplateTestNICKernelEstimator, unittest.TestCase):
    def setUp(self):
        estimator_class = NICKernelRegressor
        init_default_params = {"missing_label": MISSING_LABEL,
                               }
        fit_default_params = {"X": np.zeros((3, 1)), "y": [0.5, 0.6, MISSING_LABEL]}
    ...
    def test_predict(self):
        ...
        X = np.zeros((500, 1))
        y = norm.rvs(
            loc=1.24, scale=0.0245, size=500, random_state=0
        )
...
class TestNadarayaWatsonRegressor(TemplateTestNICKernelEstimator, unittest.TestCase):
    def setUp(self):
