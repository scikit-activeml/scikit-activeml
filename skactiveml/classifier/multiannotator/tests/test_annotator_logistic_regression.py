...
class TestAnnotatorLogisticRegression(unittest.TestCase):
    ...
    def test_init_param_tol(self):
        ...
        lr = AnnotatorLogisticRegression(tol=1.e-10)
    ...
    def test_init_param_annot_prior_full(self):
        ...
        self.assertRaises(ValueError, lr.fit, X=self.X, y=self.y)
        lr = AnnotatorLogisticRegression(annot_prior_full=[1, 1],
                                         missing_label='nan')
        self.assertRaises(ValueError, lr.fit, X=self.X, y=self.y)
    ...
    def test_init_param_annot_prior_diag(self):
        ...
        self.assertRaises(ValueError, lr.fit, X=self.X, y=self.y)
        lr = AnnotatorLogisticRegression(annot_prior_diag=[0, 0, -0.1],
                                         missing_label='nan')
        self.assertRaises(ValueError, lr.fit, X=self.X, y=self.y)
    ...
    def test_fit(self):
        lr = AnnotatorLogisticRegression(random_state=0, missing_label='nan',
                                         classes=['tokyo', 'paris'],
                                         solver='Nelder-Mead')
