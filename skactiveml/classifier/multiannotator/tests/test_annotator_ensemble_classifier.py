...
class TestAnnotatorEnsembleClassifier(unittest.TestCase):
    ...
    def test_init_param_estimators(self):
        ...
        clf = AnnotatorEnsembleClassifier(estimators=[('ParzenWindowClassifier', ParzenWindowClassifier(missing_label=0))])
        self.assertRaises(TypeError, clf.fit, X=self.X, y=self.y)
        clf = AnnotatorEnsembleClassifier(estimators=
                                 [('ParzenWindowClassifier', ParzenWindowClassifier(missing_label='a'))])
        self.assertRaises(TypeError, clf.fit, X=self.X, y=self.y)
        clf = AnnotatorEnsembleClassifier(classes=[0, 1],
                                          estimators=[('ParzenWindowClassifier', ParzenWindowClassifier(classes=[0, 2]))])
        self.assertRaises(ValueError, clf.fit, X=self.X, y=self.y)
        clf = AnnotatorEnsembleClassifier(estimators=[('ParzenWindowClassifier', ParzenWindowClassifier(classes=[0, 1]))])
        self.assertRaises(ValueError, clf.fit, X=self.X, y=self.y)
    ...
    def test_fit(self):
        ...
        clf = AnnotatorEnsembleClassifier(estimators=[('ParzenWindowClassifier', pwc)],
                                          classes=[1, 2])
        ...
        clf = AnnotatorEnsembleClassifier(estimators=[('ParzenWindowClassifier', pwc), ('GNB', gnb)],
