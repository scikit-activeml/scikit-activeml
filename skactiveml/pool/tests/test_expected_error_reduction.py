...
class TestExpectedErrorReduction(unittest.TestCase):
    def test__estimate_error_for_candidate(self):
        ...
        self.assertRaises(NotImplementedError, qs._estimate_error_for_candidate,
    ...
    def test__concatenate_samples(self):
        ...
        X_full, y_full, w_full, w_eval, idx_train, idx_cand, idx_eval = \
            qs._concatenate_samples(
                X, y, None, cand, sample_weight_cand,
                X_eval, sample_weight_eval)
        np.testing.assert_array_equal(np.ones(len(idx_train)),
                                      w_full[idx_train])
        ...
        self.assertRaises(ValueError, qs._concatenate_samples,
                          X, y, np.ones(len(X)+1),
                          cand, None,
                          X_eval, None)
        self.assertRaises(ValueError, qs._concatenate_samples,
                          X, y, None,
                          cand, np.ones(len(cand)+1),
                          X_eval, None)
        self.assertRaises(ValueError, qs._concatenate_samples,
                          X, y, None,
                          cand, None,
                          X_eval, np.ones(len(X_eval)+10))
        self.assertRaises(ValueError, qs._concatenate_samples,
                          X, y, None,
                          cand, None,
                          None, np.ones(len(X_eval)+10))

    def test__risk_estimation(self):
        def risk_estimation_slow(prob_true, prob_pred, cost_matrix,
                                 sample_weight):
...
class TestValueOfInformationEER(TemplateTestExpectedErrorReduction, unittest.TestCase):
    ...
    def test_init_param_normalize(self):
        ...
        self.assertRaises(
            TypeError, qs.query, clf=self.clf,
            X=self.X, y=self.y, normalize='string'
        )
    ...
    def test_query(self):
        ...
        for msg, consider_unlabeled, consider_labeled, \
            candidate_to_labeled, substract_current, normalize, \
        ...
        # idxs, utils = qs.query(candidates=candidates, clf=DummyClf(), X=X, y=y,
        #                       return_utilities=True)
