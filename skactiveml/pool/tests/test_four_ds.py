...
class TestFourDs(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.clf = MixtureModelClassifier(
            mixture_model=mixture_model, classes=np.unique(self.y),
            missing_label=-1
        )
    ...
    def test_query_param_fit_clf(self):
        ...
        for fit_clf in ['string', None]:
            self.assertRaises(
                TypeError, al4ds.query, X=self.X, y=self.y_unlblb,
                clf=self.clf, fit_clf='string'
            )
    ...
    def test_query_param_sample_weight(self):
        ...
        for sample_weight in [np.ones(1), 'test']:
            self.assertRaises(
                ValueError, al4ds.query, X=self.X, y=self.y_unlblb,
    ...
    def test_query(self):
        ...
        query_indices, utilities = al4ds.query(
            X=self.X, y=self.y_unlblb, clf=clf, batch_size=3, fit_clf=False,
            return_utilities=True
        )
        ...
        query_indices, utilities = al4ds.query(
            X=self.X, y=self.y_unlblb, clf=clf, fit_clf=False,
            batch_size=len(self.X) + 1, return_utilities=True
        )
