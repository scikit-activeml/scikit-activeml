...
from skactiveml.pool._epistemic_uncertainty_sampling import _interpolate, \
...
class TestEpistemicUncertaintySampling(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.clf = ParzenWindowClassifier(classes=self.classes, random_state=self.random_state)
        self.kwargs = dict(candidates=self.candidates, X=self.X, y=self.y)
        self.kwargs_MISSING_LABEL = dict(candidates=self.candidates, X=self.X,
    ...
    def test_query_param_sample_weight(self):
        selector = EpistemicUncertaintySampling()
        sample_weight_list = [
            'string', self.candidates, self.candidates[:-2],
    ...
    def test_epistemic_uncertainty_pwc(self):
        ...
        self.assertRaises(ValueError, selector.query,
                          clf=ParzenWindowClassifier(classes=[0, 1, 2]), **self.kwargs)
    ...
    def test_epistemic_uncertainty_logreg(self):
        ...
        self.assertRaises(ValueError, _epistemic_uncertainty_logreg,
                          X_cand=self.candidates, X=self.X, y=self.y, clf=clf)
        ...
        self.assertRaises(TypeError, _epistemic_uncertainty_logreg,
                          X_cand=self.candidates, X=self.X, y=self.y,
                          clf=self.clf)
    ...
    def test_query(self):
        ...
        clf = ParzenWindowClassifier(classes=self.classes, random_state=self.random_state)
        selector = EpistemicUncertaintySampling()
