...
class TestMultiAnnotWrapper(unittest.TestCase):
    ...
    def test_query(self):
        ...
        best_cand_indices, utilities = wrapper.query(self.X_cand, self.X, y, A_cand=self.A_cand, batch_size=3,
                                                     return_utilities=True, random_state=self.random_state)
        ...
        best_cand_indices, utilities = wrapper.query(self.X_cand, A_cand=self.A_cand, batch_size=3,
                                                     return_utilities=True, random_state=self.random_state)
    ...
    def check_availability(self, best_cand_indices):
        ...
        for i in range(best_value_indices.shape[0]):
            self.assertTrue(self.A_cand[best_value_indices[i], best_annotator_indices[i]])