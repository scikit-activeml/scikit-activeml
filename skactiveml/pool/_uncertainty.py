...
class UncertaintySampling(PoolBasedQueryStrategy):
    ...
    def query(self, X_cand, X, y, return_utilities=False, **kwargs):
        ...
        # get the best insance and return it
        best_indices = np.ndarray(shape=(1), buffer=np.argmax(utilities), dtype=int)
        if return_utilities:
            return best_indices, utilities.reshape(1,len(utilities))
        else:
            return best_indices
