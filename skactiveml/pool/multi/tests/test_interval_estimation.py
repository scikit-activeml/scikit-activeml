...
class TestIEThresh(unittest.TestCase):
    ...
    def test_init_param_alpha(self):
        ie_thresh = IEThresh(clf=self.clf, alpha=0.0, random_state=0)
        self.assertRaises(ValueError, ie_thresh.query, X_cand=self.X,
                          A_cand=self.A_cand, X=self.X, y=self.y)
        ie_thresh = IEThresh(clf=self.clf, alpha=1.0, random_state=0)
        self.assertRaises(ValueError, ie_thresh.query, X_cand=self.X,
                          A_cand=self.A_cand, X=self.X, y=self.y)
        ie_thresh = IEThresh(clf=self.clf, alpha='test', random_state=0)
        self.assertRaises(TypeError, ie_thresh.query, X_cand=self.X,
                          A_cand=self.A_cand, X=self.X, y=self.y)

    def test_init_param_epsilon(self):
        ie_thresh = IEThresh(clf=self.clf, epsilon=-0.1, random_state=0)
        self.assertRaises(ValueError, ie_thresh.query, X_cand=self.X,
                          A_cand=self.A_cand, X=self.X, y=self.y)
        ie_thresh = IEThresh(clf=self.clf, alpha=1.1, random_state=0)
        self.assertRaises(ValueError, ie_thresh.query, X_cand=self.X,
                          A_cand=self.A_cand, X=self.X, y=self.y)
        ie_thresh = IEThresh(clf=self.clf, epsilon='test', random_state=0)
        self.assertRaises(TypeError, ie_thresh.query, X_cand=self.X,
                          A_cand=self.A_cand, X=self.X, y=self.y)

    def test_init_param_clf(self):
        ie_thresh = IEThresh(clf=None)
        self.assertRaises(TypeError, ie_thresh.query, X_cand=self.X,
                          A_cand=self.A_cand, X=self.X, y=self.y)
    ...
    def test_query_param_X_cand(self):
        ...
        self.assertRaises(ValueError, ie_thresh.query,
                          X_cand=np.ones((10, 10)), X=self.X, y=self.y,
                          A_cand=np.ones((10, self.y.shape[1])))
        self.assertRaises(ValueError, ie_thresh.query,
                          X_cand=np.ones(10), X=self.X, y=self.y,
                          A_cand=np.ones((10, self.y.shape[1])))
        self.assertRaises(ValueError, ie_thresh.query,
                          X_cand='test', X=self.X, y=self.y,
                          A_cand=np.ones((10, self.y.shape[1])))

    def test_query_param_A_cand(self):
        ie_thresh = IEThresh(clf=self.clf)
        self.assertRaises(ValueError, ie_thresh.query,
                          X_cand=self.X, X=self.X, y=self.y,
    ...
    def test_query_param_X(self):
        ie_thresh = IEThresh(clf=self.clf)
        self.assertRaises(ValueError, ie_thresh.query,
                          X=np.ones((10, 10)), X_cand=self.X, y=self.y,
                          A_cand=self.A_cand)
        self.assertRaises(ValueError, ie_thresh.query,
                          X=np.ones(10), X_cand=self.X, y=self.y,
                          A_cand=self.A_cand)
        self.assertRaises(ValueError, ie_thresh.query,
                          X='test', X_cand=self.X, y=self.y,
                          A_cand=self.A_cand)

    def test_query_param_y(self):
        ie_thresh = IEThresh(clf=self.clf)
        self.assertRaises(ValueError, ie_thresh.query,
                          X_cand=self.X, X=self.X, y=np.ones(len(self.X)),
                          A_cand=self.A_cand)
        self.assertRaises(ValueError, ie_thresh.query,
