...
class FourDS(PoolBasedQueryStrategy):
    ...
    lmbda : float between 0 and 1, optional (default=min((batch_size-1)*0.05, 0.5))
        For the selection of more than one sample within each query round, 4DS uses a diversity measure to avoid
        the selection of redundant samples whose influence is regulated by the weighting factor 'lmbda'.
    ...
    References
    ---------
    [1] Reitmaier, T., & Sick, B. (2013). Let us know your decision: Pool-based active training of a generative
        classifier with the selection strategy 4DS. Information Sciences, 230, 106-131.
    """

    def __init__(self, clf, batch_size=1, lmbda=None, random_state=None):
        super().__init__(random_state=random_state)
        if not isinstance(clf, CMM):
            raise TypeError("'clf' must be a 'CMM' but got {}".format(type(clf)))
        self.clf = clf
    ...
    def query(self, X_cand, X, y, return_utilities=False, **kwargs):
        ...
        Parameters
        ----------
        X_cand : {array-like, sparse matrix}, shape (n_cand_samples, n_features)
        ...
        utilities: numpy.ndarray, shape (n_cand_samples) for batch_size=1 else (n_cand_samples, batch_size)
            The utilities of all instances of `X_cand` after each selection step (if return_utilities=True).
        """
        ...
        if len(X_cand) < self.batch_size:
            warnings.warn(
                "'batch_size={}' is larger than number of candidate samples in 'X_cand'. Set 'batch_size={}'".format(
        ...
        distance_cand = np.log((P_cand_sorted[:, -1] + 1.e-5) / (P_cand_sorted[:, -2] + 1.e-5))
        distance_cand = (distance_cand - np.min(distance_cand)) / (np.max(distance_cand) - np.min(distance_cand))

        # compute densities according to Eq. 10 in [1]
        density_cand = self.clf.mixture_model.score_samples(X_cand)
        density_cand = (density_cand - np.min(density_cand)) / (np.max(density_cand) - np.min(density_cand))
        ...
        distribution_cand = np.maximum(np.zeros_like(distribution_cand), distribution_cand)
        distribution_cand = 1 - np.sum(distribution_cand, axis=1)

        # compute rho according to Eq. 15  in [1]
        diff = np.sum(np.abs(self.clf.mixture_model.weights_ - np.mean(R_lbld, axis=0)))
        rho = min(1, diff)

        # compute e_dwus according to Eq. 13  in [1]
        e_dwus = np.mean((1 - P_cand_sorted[:, -1]) * density_cand)

        # coefficient normalization such alpha, beta, and rho sum up to one
        alpha = (1 - rho) * e_dwus
        beta = 1 - rho - alpha

        # compute utilities to select sample
        utilities = np.empty((len(X_cand), batch_size), dtype=float)
        utilities[:, 0] = alpha * (1 - distance_cand) + beta * density_cand + rho * distribution_cand
        ...
        if batch_size > 1:
            ...
            # normalization of the coefficients alpha, beta, and rho such that these coefficients plus
            # lmbda sum up to one
            ...
            for i in range(1, batch_size):
                ...
                R_sum = R_cand + np.sum(R_cand[is_selected], axis=0, keepdims=True) + R_lbld_sum
                ...
                diversity_cand = - np.log(density_cand + np.sum(density_cand[is_selected])) / (len(query_indices) + 1)
                diversity_cand = (diversity_cand - np.min(diversity_cand)) / (
                        np.max(diversity_cand) - np.min(diversity_cand))

                # compute utilities to select sample
                utilities[:, i] = alpha * (1 - distance_cand) + beta * density_cand + self.lmbda * diversity_cand \
                                  + rho * distribution_cand
                utilities[is_selected, i] = - np.inf
                query_indices = np.append(query_indices, rand_argmax(utilities[:, i]))
                is_selected[query_indices] = True
