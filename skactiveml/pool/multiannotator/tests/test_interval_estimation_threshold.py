...
class TestIntervalEstimationAnnotModel(unittest.TestCase):
    ...
    def test_predict_annot_perf_param_X(self):
        ...
        self.assertRaises(ValueError, ie_model.predict_annotator_perf,
                          X=np.ones(2))
...
class TestIntervalEstimationThreshold(unittest.TestCase):
    ...
    def test_init_param_alpha(self):
        ...
        self.assertRaises(ValueError, ie_thresh.query, candidates=self.X,
                          clf=self.clf, annotators=self.A_cand, X=self.X,
                          y=self.y)
        ...
        self.assertRaises(TypeError, ie_thresh.query, candidates=self.X,
                          clf=self.clf, annotators=self.A_cand, X=self.X,
                          y=self.y)
    ...
    def test_query_param_candidates(self):
        ...
        self.assertRaises(ValueError, ie_thresh.query,
                          clf=self.clf, candidates=np.ones((10, 10)), X=self.X,
                          y=self.y, annotators=np.ones((10, self.y.shape[1])))
        self.assertRaises(ValueError, ie_thresh.query,
                          candidates=np.ones(10).reshape(1, -1), clf=self.clf,
                          X=self.X, y=self.y,
                          annotators=np.ones((10, self.y.shape[1])))
        self.assertRaises(ValueError, ie_thresh.query,
                          candidates='test', clf=self.clf, X=self.X, y=self.y,
                          annotators=np.ones((10, self.y.shape[1])))

    def test_init_param_clf(self):
        ie_thresh = IntervalEstimationThreshold()
        self.assertRaises(TypeError, ie_thresh.query, candidates=self.X,
                          clf=None, annotators=self.A_cand, X=self.X, y=self.y)

    def test_query_param_annotators(self):
        ie_thresh = IntervalEstimationThreshold()
        self.assertRaises(ValueError, ie_thresh.query,
                          candidates=self.X, clf=self.clf, X=self.X, y=self.y,
    ...
    def test_query_param_X(self):
        ie_thresh = IntervalEstimationThreshold()
        self.assertRaises(ValueError, ie_thresh.query,
                          X=np.ones((10, 10)), candidates=self.X, clf=self.clf,
                          y=self.y, annotators=self.A_cand)
        self.assertRaises(ValueError, ie_thresh.query,
                          X=np.ones(10), candidates=self.X, clf=self.clf, y=self.y,
                          annotators=self.A_cand)
        self.assertRaises(ValueError, ie_thresh.query,
                          X='test', candidates=self.X, clf=self.clf, y=self.y,
                          annotators=self.A_cand)

    def test_query_param_y(self):
        ie_thresh = IntervalEstimationThreshold()
        self.assertRaises(ValueError, ie_thresh.query,
                          candidates=self.X, clf=self.clf, X=self.X,
                          y=np.ones(len(self.X)), annotators=self.A_cand)
        self.assertRaises(ValueError, ie_thresh.query,
                          candidates=self.X, clf=self.clf, X=self.X,
                          annotators=self.A_cand,
        ...
        self.assertRaises(TypeError, ie_thresh.query,
                          candidates=self.X, X=self.X, annotators=self.A_cand,
                          y='test')
    ...
    def test_query_param_sample_weight(self):
        ...
        self.assertRaises(ValueError, ie_thresh.query, candidates=self.X,
                          X=self.X, clf=self.clf, annotators=self.A_cand,
    ...
    def test_query_param_batch_size(self):
        ...
        for wrong_batch_size, error in [(0, ValueError),
                                        ('test', ValueError),
                                        (None, TypeError)]:
            self.assertRaises(error, ie_thresh.query, candidates=self.X,
                              clf=self.clf, annotators=self.A_cand, X=self.X,
    ...
    def test_query(self):
        ...
        for b_in, b_act, n in zip(batch_sizes, actual_batch_sizes, n_samples):
            query_indices, utilities = ie_thresh.query(candidates=self.X,
            ...
            query_indices = ie_thresh.query(candidates=self.X,
                                            clf=AnnotatorLogisticRegression(),
                                            X=self.X,
                                            y=self.y, annotators=self.A_cand,
                                            batch_size=b_in)
        ...
        query_indices, utilities = ie_thresh.query(candidates=self.X, X=self.X,
                                                   clf=AnnotatorLogisticRegression(),
                                                   y=self.y,
                                                   annotators=A_cand,
                                                   return_utilities=True)
        ...
        query_indices = ie_thresh.query(candidates=self.X, X=self.X,
                                        clf=AnnotatorLogisticRegression(),
                                        y=self.y, annotators=A_cand)
        self.assertEqual(len(query_indices), 0)
    ...
    def test_query_with_variant_available_annotators(self):
        ...
        query_indices, utilities = ie_thresh.query(candidates=X_cand,
                                                   clf=AnnotatorLogisticRegression(),
                                                   X=self.X, y=self.y,
                                                   annotators=A_cand,
                                                   return_utilities=True,
