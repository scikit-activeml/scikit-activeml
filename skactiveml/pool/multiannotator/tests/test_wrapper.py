...
class TestSingleAnnotWrapper(unittest.TestCase):
    ...
    def test_init_param_strategy(self):
        wrapper = SingleAnnotWrapper(MixtureModelClassifier())
    ...
    def test_init_param_y_aggregate(self):
        ...
        wrapper = SingleAnnotWrapper(random, y_aggregate='string',
                                     random_state=self.random_state)

        self.assertRaises(TypeError, wrapper.query, self.X, self.y,
                          candidates=self.X_cand, annotators=self.A_cand,
                          return_utilities=True)

        dummy_function = lambda x, y: majority_vote(x)

        random = RandomSampling(self.random_state)
        wrapper = SingleAnnotWrapper(random, y_aggregate=dummy_function,
                                     random_state=self.random_state)
    ...
    def test_init_param_random_state(self):
        random = RandomSampling(self.random_state)
        wrapper = SingleAnnotWrapper(random, random_state='string')
    ...
    def test_query_param_candidates(self):
        random = RandomSampling(self.random_state)
        X_cand = [1, 0, 2, 4]
        wrapper = SingleAnnotWrapper(random, random_state=self.random_state)

        self.assertRaises(ValueError, wrapper.query, self.X, self.y,
                          candidates=X_cand, annotators=self.A_cand,
                          return_utilities=True)

        self.assertRaises(TypeError, wrapper.query, self.X, self.y,
                          candidates=0, annotators=self.A_cand,
                          return_utilities=True)

    def test_query_param_query_params_dict(self):

        uncertainty = UncertaintySampling(method='entropy')
        wrapper = SingleAnnotWrapper(uncertainty, random_state=self.random_state)

        self.assertRaises(TypeError, wrapper.query, self.X, self.y,
                          candidates=self.X_cand, annotators=self.A_cand,
                          query_params_dict="string", return_utilities=True)
    ...
    def test_query_param_y(self):
        ...
        self.assertRaises(ValueError, wrapper.query, self.X, y,
                          candidates=self.X_cand, annotators=self.A_cand,
                          query_params_dict={'clf': clf}, return_utilities=True)
    ...
    def test_query_param_X(self):
        ...
        self.assertRaises(ValueError, wrapper.query, X, self.y,
                          candidates=self.X_cand, annotators=self.A_cand,
                          query_params_dict={'clf': clf}, return_utilities=True)
    ...
    def test_query_param_annotators(self):
        ...
        self.assertRaises(ValueError, wrapper.query, self.X, self.y,
                          candidates=self.X_cand, annotators=annotators,
                          return_utilities=True)
    ...
    def test_query_one_annotator_per_sample_batch_size_one(self):
        ...
        wrapper = SingleAnnotWrapper(uncertainty, y_aggregate=y_aggregate,
                                     random_state=self.random_state)
    ...
    def test_query_one_annotator_per_sample_batch_size_five(self):
        ...
        re_val = wrapper.query(self.X, self.y, candidates=self.X_cand,
                               annotators=self.A_cand, batch_size=5,
                               return_utilities=True)
    ...
    def test_query_batch_size_too_large(self):
        ...
        re_val = wrapper.query(self.X, self.y, batch_size=4,
                               return_utilities=True)
    ...
    def test_query_three_n_annotators_per_sample_batch_size_five_mismatch(self):
        ...
        A_cand = np.array([[True, True, True],
                           [True, False, True]])

        wrapper = SingleAnnotWrapper(random, random_state=self.random_state)

        re_val = wrapper.query(self.X, self.y, candidates=X_cand,
                               annotators=A_cand, batch_size=5,
                               n_annotators_per_sample=3,
                               return_utilities=True)
    ...
    def test_query_unavailable_annotators(self):
        ...
        A_cand = np.array([[False, False, False],
                           [True, True, True],
                           [False, False, False],
                           [True, False, True]])
    ...
    def test_query_custom_annotator_special_preference(self):
        ...
        re_val = wrapper.query(self.X, self.y, candidates=X_cand,
                               ...
                               A_perf=A_perf, return_utilities=True)
    ...
    def test_query_custom_annotator_general_equal_preference(self):
        ...
        re_val = wrapper.query(self.X, self.y, candidates=X_cand, batch_size=6,
                               n_annotators_per_sample=3, A_perf=A_perf,
                               return_utilities=True)
