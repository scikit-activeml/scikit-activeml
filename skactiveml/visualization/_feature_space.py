...
from ._misc import mesh, check_bound, _get_boundary_args, \
    _get_confidence_args, _get_contour_args, _get_cmap
from ..base import QueryStrategy, SingleAnnotatorPoolQueryStrategy, \
...
def plot_utilities(qs, X, y, candidates=None, **kwargs):
    ...
    **kwargs
    ...
    return _general_plot_utilities(qs=qs, X=X, y=y, candidates=candidates,
                                   **kwargs)
...
def _general_plot_utilities(qs, X, y, candidates=None, **kwargs):
    ...
    **kwargs
    ...
    if n_annotators is None:
        if axes is not None:
            raise TypeError(
                '`axes` can be only used in the multi-annotator setting. Use `ax` instead.')
        if ax is None:
        ...
        if ax is not None:
            raise ValueError(
                '`ax` can be only used in the single-annotator setting. Use `axes` instead.')
        if axes is None:
    ...
    if n_annotators is not None and len(axes) != n_annotators:
        raise ValueError('`axes` must contain one `Axes` object for each '
