...
def mesh(bound, res):
    ...
    Returns
    -------
    X_mesh : np.ndarray of shape (res, res)
        Mesh grid over x.
    Y_mesh : np.ndarray of shape (res, res)
        Mesh grid over y.
    mesh_samples : np.ndarray of shape (res*res, 2)
    ...
    mesh_samples = np.array([X_mesh.reshape(-1), Y_mesh.reshape(-1)]).T
    return X_mesh, Y_mesh, mesh_samples
