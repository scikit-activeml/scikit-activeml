...
def check_missing_label(missing_label, target_type=None, name=None):
    ...
    if not is_number and not is_character and not is_None:
        raise TypeError(
            "'missing_label' has type '{}', but must be a either a number, a string, np.nan, or None.".format(
                type(missing_label)))
    if target_type is not None:
        ...
        if (is_character_type and is_number) or (is_number_type and is_character) or (is_object_type and not is_None):
            name = 'target object' if name is None else str(name)
            raise TypeError(
                "'missing_label' has type '{}' and is not compatible to the type '{}' of '{}'.".format(
...
def check_classes(classes):
    ...
    try:
        ...
        raise TypeError("'classes' must be uniformly strings or numbers. Got {}".format(types))
...
def check_cost_matrix(cost_matrix, n_classes):
    ...
    cost_matrix = check_array(np.asarray(cost_matrix, dtype=float), ensure_2d=True)
    if cost_matrix.shape != (n_classes, n_classes):
        raise ValueError(
            "'cost_matrix' must have shape ({}, {}). Got {}.".format(n_classes, n_classes, cost_matrix.shape))
    return cost_matrix
