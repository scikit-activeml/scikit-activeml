...
def conditional_expect(X, func, cond_est, method=None, n_monte_carlo=None,
                       scipy_dict=None, random_state=None, include_x=False,
                       include_idx=False, vector_func=False):
    ...
    vector_func: bool, optional (default=False)
        If `vector_func` is `True`, the integration values are passed as a hole
    ...
    if vector_func:
        if method == 'monte_carlo':
            potential_y_vals = cond_est.sample_y(X=X,
                                                 n_rv_samples=n_monte_carlo,
            ...
            for idx in np.arange(len(X)):
                ...
                expectation[idx] = cond_dist.expect(
                    lambda y: func(*arg_filter(np.array([idx]),
                                               np.array([x]),
                                               np.array([y]))),
                    **scipy_dict
                )
        ...
        if method == 'monte_carlo':
            for idx, x in enumerate(np.arange(len(X))):
                potential_y_vals = cond_est.sample_y(
                    X=[x], n_rv_samples=n_monte_carlo,
            ...
            for idx, x in enumerate(np.arange(len(X))):
                y_val = cond_est.predict([x]).flatten()
                expectation[idx] = func(*arg_filter(idx, x, y_val))
        elif method == 'scipy':
            for idx, x in enumerate(np.arange(len(X))):
                cond_dist = cond_est.estimate_conditional_distribution([x])
                expectation[idx] = cond_dist.expect(
                    lambda y: func(*arg_filter(idx, x, y)),
                    **scipy_dict
                )
