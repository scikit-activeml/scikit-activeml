...
class RandomSampler(StreamBasedQueryStrategy):
    ...
    def query(self, X_cand, return_utilities=False, adapt_query_strategy=True, **kwargs):
        X_cand = check_array(X_cand, force_all_finite=False)
        
        utilities = self.random_state.random_sample(len(X_cand))
        
        sampled_indices = self.budget_manager.sample(utilities, adapt_budget_manager=adapt_query_strategy)
...
class PeriodicSampler(StreamBasedQueryStrategy):
    ...
    def query(self, X_cand, return_utilities=False, adapt_query_strategy=True, **kwargs):
        ...
        _, utilities = self.budget_manager.sample(np.ones(len(X_cand)), adapt_budget_manager=adapt_query_strategy, return_budget_left=True)
