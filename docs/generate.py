...
def generate_strategy_overview_rst(gen_path, examples_data={}):
    ...
    with open(os.path.join(gen_path, "strategy_overview.rst"), "w") as file:
        ...
        file.write("\n")
        file.write(
            "In the following you'll find summaries of all implemented"
    ...
    for tab, cats in examples_data.items():
        ...
        with open(path, "w") as file:
            ...
            file.write("\n")
            file.write(
                f"This is an overview of all implemented AL strategies. "
...
def generate_examples(gen_path, package, json_path):
    ...
    for row in table:
        for tab in examples_data:
            ...
            examples_data[tab][cat] = np.append(
                examples_data[tab][cat], [row[:3]], axis=0
            )
        ...
        for line in template:
            if "#_" in line:
                if "#_import" in line:
                    ...
                    if (
                        "random_state" not in data["init_params"].keys()
                        and "random_state"
